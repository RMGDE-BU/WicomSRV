...
$Id: IdModbusServer.pas,v 1.38 2014/01/14 15:21:20 plpolak Exp $

// 25.10.2010  GD  Erweiterung um LOG-Funktionalität
//                 Erweiterung um mbfWriteOneCoil
// 28.02.2023  WW  Bugfix Prüfung auf FMaxRegister in ReadCommand
...
type
  TModBusServerLogProc = procedure(const sLogType, sPeerIP: string;  // 25.10.2010
    const pByteBuffer: array of byte; const iSize: integer) of object;
...
{$ENDIF}
  private
    ...
    FOnLog: TModBusServerLogProc;  // 25.10.2010
    FPause: Boolean;
    FUnitID: Byte;
    ...
    procedure IdModBusServerExecute(AThread: TIdPeerThread);  // 25.10.2010
  {$IFDEF DMB_INDY10}
  ...
  { public properties }
    ...
    property OnLog: TModBusServerLogProc read FOnLog write FOnLog;  // 25.10.2010
  published
...
{$ENDIF}
  ...
  FOnLog := nil;  // 25.10.2010  // Keine Log-Behandlung zugewiesen
  FPause := False;
...
var
  F: TextFile;
begin
  if (FLogEnabled) and ((FLogFile <> '') or (Assigned(FOnLog))) then begin  // 25.10.2010
    FLogCriticalSection.Enter;
    try
      // 25.10.2010  // Ggf. Ausgabe-Prozedure füttern
      if (Assigned (FOnLog)) then FOnLog(LogType, PeerIP, ByteBuffer, Size);

      // Ggf. Asgabedatei füttern
    ...
    case ReceiveBuffer.FunctionCode of
      ...
      mbfReadInputBits:
        begin
          ...
          if ((iRegNr < FMinRegister) or ((iRegNr + iCount - 1) > FMaxRegister)) then  // Bugfix ... - 1; 28.02.2023, WW
          {$IFDEF DMB_INDY10}
...
// 25.10.2010
// Notfall "OnExecute"-Methode (sonst Verbindungsabbruch)
procedure TIdModBusServer.IdModBusServerExecute(AThread: TIdPeerThread);
begin
  Sleep(1);
end;
...
{$ENDIF}
var
  bHasOnExecute : boolean;   // 25.10.2010
begin
  Result := False;
  if not FPause then
  begin
    bHasOnExecute := Assigned(Self.OnExecute);
    if (not bHasOnExecute) then Self.OnExecute := Self.IdModBusServerExecute;
    try
    {$IFDEF DMB_INDY10}
    ...
    {$ENDIF}
      ...
      if (not bHasOnExecute) then Self.OnExecute := nil;
    end;
  end;
end;
