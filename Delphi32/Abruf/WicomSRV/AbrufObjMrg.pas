...
{* 04.04.2024 WW Erweiterung für RSM200: Beim Auslesen von Zähler-Parametern  *}
{*               immer Parameter 'Zählerfaktor' mitauslesen;                  *}
{*               Erweiterung für TME400, RSM200: Auslesen der Parameter für   *}
{*               virtuelle Gerätezustands-Parameter;                          *}
{*               Umstellung lesen der Parameter-Ressourcedaten-Konfiguration  *}
{*               'Gerätezustand-Parameter' aus Feld 'Filtertyp' statt 'Para-  *}
{*               Datentyp'                                                    *}
{******************************************************************************}
Unit AbrufObjMrg;
...
Type
  ...
  private
    ...
    FParamMrgData_ZaehlerFaktor: TParamMrgData;  { Konfigurationsdaten-Record zu Parameter 'Zählerfaktor'; 04.04.2024, WW
                                                   -> Für Gerätetypen, für die eine Verrechnung der
                                                      Zähler-Parameterrohwerte mit Faktor-Parameter erforderlich ist (RSM200) }
...
{ Liste mit Befehlen zum Lesen von Parametern zusammenstellen abhängig von Gerätegruppe;
  Übergabe: Filtertyp der Parameter, mit denen Befehl(e) gebildet werden soll(en)
            ...
            Zeiger auf Liste abzurufender Parameter (allg. Parameternummern),
              ...
              -> Wenn <> nil: Übergabe 'AParaFiltertyp' nicht relevant
...
{ Liste mit Befehlen zum Lesen der Gerätezustands-Parameter zusammenstellen
  ...
  Übergabe/Rückgabe: Parameter-Befehlliste }
begin
  { Aktueller Gerätezustand EK 260, EK 280, DL 240, DL 220, DL 210, MRG 905/910: }
...
{--------------------------------------------------------}
{ Konfigurationsdaten-Record zu Parameter 'Zählerfaktor' aus Parameternummern-
...
begin
  ...
  else  // übrige Gerätetypen
    sParamNrAllg:='';  // Nicht erforderlich
  end;

  if sParamNrAllg <> '' then begin
    { Parameter "Zähler-Faktor" über allg. Parameternummer ermitteln: }
    ...
    // In FParamMrgData_ZaehlerFaktor für den Abruf merken:
...
{ Modbus-Register-Requestliste zum Lesen von Parametern zusammenstellen (basierend
  ...
  Übergabe: Filtertyp der Parameter, aus denen die Modbus-Register-Requestliste
  ...
  {----------------------------------------------------------------------------}
  { Parameter der Registerdaten-Liste hinzufügen;
    Ergebnis: True, wenn Hinzufügen erfolgreich }
  var
    sParaNrMrg: string;
    sParaAdrMrg: string;
    iParaAdr: integer;
        ...
        begin
          if FParamMrgKonfigList.Count > 0 then begin  // 09.07.2021, WW
            ...
            for i:=0 to FParamMrgKonfigList.Count-1 do begin
              ...
              if bAdd then begin
                // Prüfen auf Zähler- und Zählerfaktor-Parameter:
                case FMrgDefData.MrgTyp of
                  mrgtyp_RSM200_VCF,
            ...
            // Den erforderlichen Zählerfaktor-Parameter zusätzlich hinzufügen,
            // wenn Zähler-Parameter enthalten sind; 04.04.2024, WW:
...
function TMRGAbruf.FillMBRegisterRequestListe_Geraetezustand (AAllgParaNr: string;
  RRL: TRegisterRequestList): boolean;
{--------------------------------------------------------------------------------}
{ Modbus-Register-Requestliste zum Lesen der Gerätezustands-Parameter zusammen-
  stellen abhängig von allgemeiner Parameternummer;
  Übergabe: Allgemeine Parameternummer für Gerätezustand
  Übergabe/Rückgabe: Register-Requestliste
  Ergebnis: True, wenn Zusammenstellen erfolgreich }
begin
  { Aktueller Gerätezustand TME400, RSM200: }
...
begin
  ...
  { für Abruf von Geräten mit Verrechnung der Zähler-Parameterrohwerte mit
...
{ Parameter-Konfigurationsdaten für abzurufendes MRG initialisieren:
  ...
  -> Konfigurationsdaten-Record zu Parameter 'Zählerfaktor' ermitteln, wenn für
     Gerätetyp erforderlich
...
begin
  ...
  { Konfigurationsdaten-Record zu Parameter 'Zählerfaktor' ermitteln: }
              ...
              if AlleParameter AND not Assigned (AllgParaNrListe) then begin  // alle
                ...
                try
                  ...
                  if Assigned (AllgParaNrListe) then
                    ...
                    slParaAllg:=TStringList.Create;  // Temp-Liste für allg. Parameternummer
                    try
                  ...
                  else begin  { virtueller Parameter (Gerätezustand); 04.04.2024, WW }
                    { Laden der Modbus-Requests zum Lesen der einzelnen
                      Gerätezustands-Parameter: }
