...
const
  ...
  RN_RenameTo  = BaseOffset + 143;      // new v1.6    Rename 
	RN_InvalidDateTime = BaseOffset + 144;  	// new v1.73
  ...
  AD_InIsOutStream = BaseOffset + 306;  // new v1.6		Add Stream
	...
	AD_NoDestDir = BaseOffset + 308;			// new 1.73
  ...
  LZ_NoZipDll  = BaseOffset + 604;
	...
	LZ_ZipDllUnloaded = BaseOffset + 606;
  ...
  LU_NoUnzDll  = BaseOffset + 704;
	...
	LU_UnzDllUnloaded = BaseOffset + 706;
  ...
  FM_Erase     = BaseOffset + 1101;     // 1.72 ?
  FM_Confirm   = BaseOffset + 1102;     // 1.72 ?

	ED_SizeTooLarge = BaseOffset + 1201;  // 1.73

	CD_NoCDOnSpan      = BaseOffset + 1301;
	CD_NoEventHndlr    = BaseOffset + 1302;
	CD_LOExtraLen      = BaseOffset + 1303;
	CD_CEExtraLen      = BaseOffset + 1304;
	CD_CEComLen        = BaseOffset + 1305;
	CD_FileName        = BaseOffset + 1306;
	CD_CEHDataSize     = BaseOffset + 1307;
	CD_Changing        = BaseOffset + 1308;
	CD_DuplFileName    = BaseOffset + 1309;
	CD_NoProtected	   = BaseOffset + 1310;
	CD_InvalidFileName = BaseOffset + 1311;
	CD_NoChangeDir		 = BaseOffset + 1312;
	CD_FileSpecSkip		 = BaseOffset + 1313;

	PR_Progress             = BaseOffset + 1400;
	PR_Archive              = PR_Progress + 1;
	PR_CopyZipFile          = PR_Progress + 2;
	PR_SFX                  = PR_Progress + 3;
	PR_Header               = PR_Progress + 4;
	PR_Finish               = PR_Progress + 5;
	PR_Copying              = PR_Progress + 6;
	PR_CentrlDir            = PR_Progress + 7;
	PR_Checking							= PR_Progress + 8;

	WZ_DropDirOnly          = BaseOffset + 1500;
	WZ_NothingToWrite				= BaseOffset + 1501;
                                                
	LD_NoDll								= BaseOffset + 650;
	LD_BadDll								= BaseOffset + 651;
	LD_DllLoaded						= BaseOffset + 652;
	LD_DllUnloaded					= BaseOffset + 653;

	RDLL_Zip								= BaseOffset + 1605;
	RDLL_Unz								= BaseOffset + 1606;   
	RDLL_ZVer								= 11607;
	RDLL_UVer								=	11608;
